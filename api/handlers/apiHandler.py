...
class ApiHandler(object):
    ...
    def sendRequest(self,call,header=None):
        ...
        try:
            ...
            else:       #if('text' in response.headers.get('Content-Type','')): #TODO: handle decoding byte files here 
    ...
    def fetchHeroesDetailed(self,**kwargs):
        ...
        return url
    def fetchHeroLore(self,**kwargs):
        url= urls.DOTAB_HERO_LORE
        return url
    def fetchItems(self,**kwargs):
        url = urls.DOTA_ITEM_INFO
        return url 
    def fetchAbilityLocalization(self,**kwargs):
        url = urls.DOTA_ABILITIES_LANG_EN
        return url
    def fetchHeroAbilities(self,hero,**kwargs):
        '''fetch ability vdf for hero name''' 
        return '{}{}.txt'.format(urls.DOTABUFF_HERO_ABILITY_BASE,hero)

    def fetchAbilityIds(self,**kwargs):
        url = urls.DOTA_ABILITY_IDS
        return url
    ...
    def fetchMatch(self,match_id):
        ''' opendota single match lookup
        :match_id: int 
        returns
        url
        ''' 
        url = f'{urls.OPEN_DOTA_BASE}{urls.OPEN_DOTA_MATCH}{match_id}'
        return url 