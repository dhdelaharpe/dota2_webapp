...
class dbHandler(object):
    ...
    def connect(self, dbName=None, collectionName=None, id=None):
        ...
        try:
            ...
            if(id is not None):
                indexes = [index['name'] for index in self.collection.list_indexes()]
                if f"{id}_1" not in indexes:
    ...
    def updateBulkData(self,data,queryOn):
        '''Create list of operations to perform bulk_write using upsert=True and execute
        :param data: (dict) data to be written
        :param queryOn: (str) parameter used in query, must exist in each entry of data and exist in db 
        ''' 
        operations=[]
        for key,value in data.items():
            update_op = {"$set": value}
            query= {queryOn: value[queryOn]}
            operations.append(UpdateOne(query,update_op,upsert=True))
        if(self.collection is not None):
            if(self.logger):
            ...
            if(operations):
                try:
                    ...
                    logger.info(f"Matched: {result.matched_count}, updated: {result.modified_count}, inserted: {result.upserted_count}") if(self.logger) else print(f"Matched: {result.matched_count}, updated: {result.modified_count}, inserted: {result.upserted_count}")
                        
                except errors.BulkWriteError as be:
                    if(self.logger):
                        logger.error(be)
                    else:
                        print(f"Bulk write error: {be}")
            ...
            self.logger.info('no collection found') if self.logger else print("no collection found")
        ...
        if self.collection is not None:
            try:
                ...
                if(many):
                    ...
                    if(self.logger):        #printing for each would slow down the application too much if used in a loop --> should use bulk_write
